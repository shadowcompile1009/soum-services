...
import { newOrdersColumn } from './helpers/columns';
...
export function NewOrdersTable() {
  ...
  const {
    ...
    limit: apiLimit,
    offset,
  } = useOrdersTable({ submodule: EOrderModules.NEW });
  ...
  const columns = useMemo(
    () =>
      getOrderColumns({
        columns: newOrdersColumn,
        queryKey: [
          QUERY_KEYS.newOrders,
          String(apiLimit),
          String(offset),
          String(search),
        ],
  ...
  if (isEmpty(newOrders) && !isLoading && isSuccess)
    return (
      <TableContainer>
  ...
  return (
    <>
      ...
      >
        {isEmpty(newOrders) && !isLoading && isSuccess ? (
          <Text color="static.gray" fontSize="smallText" fontWeight="regular">
            No orders
          </Text>
        ) : (
