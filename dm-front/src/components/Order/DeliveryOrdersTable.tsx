...
import { getDeliveryOrderColumns } from '@/components/Order/helpers/getDeliveryOrderColumns';
import { QUERY_KEYS } from '@/constants/queryKeys';
import { deliveryOrdersColumn } from './helpers/columns';
...
export function DeliveryOrdersTable() {
  ...
  const {
    ...
    limit: apiLimit,
    offset,
  } = useOrdersTableV2({ submodule: 'delivery' });
  ...
  const columns = useMemo(
    ...
    // need new deliveredOrders deps, otherwise react-table doesnt know, if we have new data
  ...
  if (isEmpty(deliveredOrders) && !isLoading && isSuccess)
    return (
      <TableContainer>
  ...
  return (
    <>
      ...
      >
        {isEmpty(deliveredOrders) && !isLoading && isSuccess ? (
          <Text color="static.gray" fontSize="smallText" fontWeight="regular">
            No orders
          </Text>
        ) : (
