import { MultiValue } from 'react-select';
import { MultiSelect } from '@/components/MultiSelect';
import { IOrderStatus } from '@/models/Order';
import { IWalletStatus } from '@/models/Wallet';
import { colors } from '@/tokens/colors';

interface FilterStatusSelectProps {
  options: (IOrderStatus | IWalletStatus)[];
  handleOnSelect: (values: MultiValue<any>) => void;
  initialValues?: (IOrderStatus | IWalletStatus)[];
  placeholder: string;
}

const styles = {
  placeholder: (provided: Record<string, unknown>) => ({
    ...provided,
    color: colors.static.gray,
    opacity: 0.6,
  }),
  control: (provided: Record<string, unknown>) => ({
    ...provided,
    minHeight: '44px',
    lineHeight: '27px',
    minWidth: '280px',
    maxWidth: '560px',
    fontFamily:
      "Inter, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif",
    fontSize: '18px',
    borderColor: colors.static.blue,
    '&:hover': {
      borderColor: colors.static.blue,
    },
  }),
};

export function FilterStatusSelect(props: FilterStatusSelectProps) {
  const { options, handleOnSelect, initialValues, placeholder } = props;

  return (
    <MultiSelect
      // @ts-ignore
      getOptionLabel={(option: IOrderStatus | IWalletStatus) =>
        option.displayName
      }
      // @ts-ignore
      getOptionValue={(option: IOrderStatus | IWalletStatus) => option.name}
      placeHolder={placeholder}
      instanceId="filter-status-select"
      // @ts-ignore
      handleOnSelect={handleOnSelect}
      options={options}
      styles={styles}
      value={initialValues}
      isSearchable={true}
    />
  );
}
